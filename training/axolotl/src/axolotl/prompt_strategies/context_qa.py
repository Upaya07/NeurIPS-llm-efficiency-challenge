...
def load_dolly(tokenizer, cfg):
    return ContextQaDollyPromptTokenizingStrategy(
        ContextDollyPrompter(),
        tokenizer,
        cfg.train_on_inputs,
        cfg.sequence_len,
    )
...
class ContextQaDollyPromptTokenizingStrategy(InstructionPromptTokenizingStrategy):
    """
    Tokenization Strategy to combine in-context article with a question and answer
    """

    def parse_instruction_fields(self, prompt) -> Tuple[str, str, str]:
        instruction = f"[INST] {prompt['instruction']}"
        context = f"Here's some context: {prompt['context']}" if len(prompt["context"]) > 0 else None
        response = f"[/INST] {prompt['response']}"
...
class ContextDollyPrompter(AlpacaPrompter):
    """
    Customized system prompted for concise QA
    """
